...
export function ScrollFade({ edge, blurTarget }: { edge: 'top' | 'bottom'; blurTarget: RefObject<View | null> }) {
  const { rawColors, isDark } = useTheme();
  const top = edge === 'top';
  return <View pointerEvents="none" style={{ position: 'absolute', left: 0, right: 0, [edge]: 0, height: 36 }}>
